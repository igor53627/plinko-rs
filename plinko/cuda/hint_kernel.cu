...
// Derive PRP key from block key: SHA256(block_key || "prp")[0..16]
...
extern "C" __global__ void hint_gen_kernel_warp(
    ...
    for (uint64_t block_idx = 0; block_idx < params.set_size; block_idx++) {
        ...
        if (in_subset) {
            uint64_t preimage = sn_inverse_batched(prp_key, hint_idx, params.chunk_size);
...
extern "C" __global__ void hint_gen_kernel(
    ...
    for (uint64_t block_idx = 0; block_idx < params.set_size; block_idx++) {
        ...
        uint64_t preimage = sn_inverse_simple(prp_key, hint_idx, params.chunk_size);

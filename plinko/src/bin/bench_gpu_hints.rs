...
fn main() -> Result<()> {
    ...
    let hint_count = args.hint_count.unwrap_or(full_total_hints - hint_start);

    // Apply max_hints limit if specified (for benchmarking)
    let (total_hints, hints_scale_factor) = if let Some(max) = args.max_hints {
        if max < hint_count {
            println!("  NOTE: Limiting hints to {} (of {}) due to memory constraints", max, hint_count);
    ...
    if production_mode {
        println!("  PRODUCTION MODE: Will save hints to {:?}", args.output.as_ref().unwrap());
        println!("  Hint range: {} to {} ({} hints)", hint_start, hint_start + total_hints, total_hints);
    }
    ...
    #[cfg(feature = "cuda")]
    {
        ...
        if production_mode {
            ...
            println!("Generating {} hints (starting at {})...", total_hints, hint_start);
            let start = Instant::now();
            let hints = gpu.generate_hints(entries, &gpu_block_keys, &hint_subsets, params)?;
            let elapsed = start.elapsed();

            println!("  Generation time: {:.3} s ({} hints)", elapsed.as_secs_f64(), hints.len());
            println!("  Throughput: {:.0} hints/sec", hints.len() as f64 / elapsed.as_secs_f64());
            ...
            println!("  Saved {} hints ({} bytes, {:.2} MB)", hints.len(), file_size, file_size as f64 / 1e6);
            println!();

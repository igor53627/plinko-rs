//! 40-byte PIR entry schema for optimized storage and GPU performance.
//!
//! This module defines the 40-byte (320-bit) entry format that provides:
//! - 17% storage reduction vs 48-byte schema
//! - 12% faster GPU hint generation (better memory bandwidth utilization)
//! - Uniform row size for both accounts and storage (simplifies PIR geometry)
//! - Cuckoo TAG for client-side collision resolution
//! - Code ID indirection for bytecode deduplication
//!
//! # Layout
//!
//! ## Account Entry (40 bytes)
//! ```text
//! ┌─────────────────┬──────────┬────────┬─────────┬─────────────┐
//! │    Balance      │  Nonce   │ CodeID │   TAG   │   Padding   │
//! │    16 bytes     │  4 bytes │ 4 bytes│ 8 bytes │   8 bytes   │
//! └─────────────────┴──────────┴────────┴─────────┴─────────────┘
//! ```
//!
//! | Field   | Size | Description                                                     |
//! |---------|------|-----------------------------------------------------------------|
//! | Balance | 16B  | ETH balance (128-bit LE), sufficient for total ETH supply ×10¹³ |
//! | Nonce   | 4B   | Transaction count (u32), supports 4B txs per account            |
//! | CodeID  | 4B   | Index into bytecode CAS, 0 = EOA (no code)                      |
//! | TAG     | 8B   | Cuckoo fingerprint: `keccak256(address)[0:8]`                   |
//! | Padding | 8B   | Zero-filled for 40-byte alignment                               |
//! | **Total** | **40B** |                                                             |
//!
//! ## Storage Entry (40 bytes)
//! ```text
//! ┌────────────────────────────────────┬─────────────────────────┐
//! │           Storage Value            │           TAG           │
//! │             32 bytes               │         8 bytes         │
...
//! | Field   | Size | Description                                                     |
//! |---------|------|-----------------------------------------------------------------|
//! | Value   | 32B  | Full 256-bit storage slot value (LE)                            |
//! | TAG     | 8B   | Cuckoo fingerprint: `keccak256(address \|\| slot_key)[0:8]`     |
//! | **Total** | **40B** | No padding needed (32 + 8 = 40 exactly)                     |

/// Entry size in bytes (40 = 2.5 × 16, optimized for storage/bandwidth)
pub const ENTRY_SIZE: usize = 40;

/// Entry size as u64 count (40 / 8 = 5)
pub const ENTRY_U64_COUNT: usize = 5;

/// Balance field size (128-bit, truncated from 256-bit)
pub const BALANCE_SIZE: usize = 16;

/// Nonce field size (32-bit, sufficient for 4B transactions)
pub const NONCE_SIZE: usize = 4;

/// Code ID field size (index into bytecode store)
pub const CODE_ID_SIZE: usize = 4;

/// TAG field size (Cuckoo fingerprint, 64-bit for ~0 collisions)
pub const TAG_SIZE: usize = 8;

/// Account padding size (for 40-byte alignment)
pub const ACCOUNT_PADDING_SIZE: usize = 8;

/// Storage value size (full 256-bit word)
pub const STORAGE_VALUE_SIZE: usize = 32;

/// Storage padding size (none needed - 32 + 8 = 40 exactly)
...
/// (address || slot_key) for storage entries. With 64 bits, collision
/// probability is ~5×10⁻²⁰, essentially zero for any practical database size.
...
/// Supports up to 4 billion unique bytecodes.
...
/// 40-byte Account Entry.
///
/// Layout:
/// - bytes 0..16:  Balance (128-bit LE, truncated from 256-bit)
/// - bytes 16..20: Nonce (32-bit LE, supports 4B transactions)
/// - bytes 20..24: Code ID (32-bit LE, index into bytecode store)
/// - bytes 24..32: TAG (8 bytes, Cuckoo fingerprint)
/// - bytes 32..40: Padding (8 bytes, zeroed)
#[derive(Clone, Copy, Debug)]
#[repr(C, align(8))]
pub struct AccountEntry40 {
    /// Account balance, truncated to 128-bit.
    ...
    /// u32 supports 4 billion transactions, sufficient for 100+ years of activity.
    pub nonce: u32,
    /// Index into bytecode store. 0 = EOA (no bytecode).
    pub code_id: CodeId,
    /// Cuckoo TAG for collision resolution.
    /// Derived from keccak256(address)[0:8].
    pub tag: Tag,
    /// Padding for 40-byte alignment.
    pub _padding: [u8; ACCOUNT_PADDING_SIZE],
}

impl Default for AccountEntry40 {
    fn default() -> Self {
        Self {
...
impl AccountEntry40 {
    ...
    ///
    /// # Arguments
    /// * `balance_256` - Full 256-bit balance (will be truncated to 128-bit)
    /// * `nonce` - Account nonce (will be truncated to u32 if > u32::MAX)
    ...
    pub fn new(balance_256: &[u8; 32], nonce: u64, code_id: CodeId, address: &[u8; 20]) -> Self {
        ...
        // Truncate nonce to u32 (with saturation for safety)
    ...
    pub fn to_bytes(&self) -> [u8; ENTRY_SIZE] {
        ...
        // bytes[32..40] remain zero (padding)
        bytes
    }
    ...
    /// Get nonce as u64 for compatibility with code expecting 64-bit nonces.
    pub fn nonce_u64(&self) -> u64 {
        self.nonce as u64
    }
}

/// 40-byte Storage Entry.
///
/// Layout:
/// - bytes 0..32:  Storage Value (256-bit LE)
/// - bytes 32..40: TAG (8 bytes, Cuckoo fingerprint)
///
/// Note: No padding needed - 32 + 8 = 40 exactly.
#[derive(Clone, Copy, Debug)]
#[repr(C, align(8))]
pub struct StorageEntry40 {
    ...
    /// Cuckoo TAG for collision resolution.
    /// Derived from keccak256(address || slot_key)[0:8].
    pub tag: Tag,
}

impl Default for StorageEntry40 {
    fn default() -> Self {
        Self {
...
mod tests {
    ...
    fn test_nonce_truncation() {
        ...
        // Normal nonce
        let entry1 = AccountEntry40::new(&balance, 12345, CodeId(0), &address);
        assert_eq!(entry1.nonce, 12345);
        assert_eq!(entry1.nonce_u64(), 12345);

        // Max u32 nonce
        let entry2 = AccountEntry40::new(&balance, u32::MAX as u64, CodeId(0), &address);
        assert_eq!(entry2.nonce, u32::MAX);

        // Nonce exceeding u32 gets saturated
        let entry3 = AccountEntry40::new(&balance, u64::MAX, CodeId(0), &address);
        assert_eq!(entry3.nonce, u32::MAX);
    }
    ...
    fn test_balance_truncation() {
        ...
        let entry = AccountEntry40::new(&balance_small, 0, CodeId(0), &[0u8; 20]);
        assert_eq!(&entry.balance[..], &balance_small[..16]);

        // Large balance (upper 128 bits set) - gets truncated
        let balance_large = [0xFFu8; 32];
        let entry2 = AccountEntry40::new(&balance_large, 0, CodeId(0), &[0u8; 20]);
    ...
    fn test_storage_no_padding() {
        // Verify storage entry has no padding field
        let entry = StorageEntry40::default();
        let bytes = entry.to_bytes();

        // All 40 bytes should be used (32 value + 8 tag)
        assert_eq!(bytes.len(), 40);

        // Value is first 32 bytes

    ...
    loop {
        ...
        for entry in walker {
            ...
            if let Some(writer) = db_writer.as_mut() {
                ...
                let entry = AccountEntry48::new(&balance_bytes, account.nonce, code_id, &addr_bytes);
            ...
            if let Some(writer) = acc_map_writer.as_mut() {
                ...
                writer.write_all(&(total_entries as u32).to_le_bytes())?;
            }
            ...
            if count_acc % 10000 == 0 {
                pb.set_message(format!("Acc: {}, Sto: 0, CodeIDs: {}", count_acc, code_store.len()));
                pb.inc(1);
            }
